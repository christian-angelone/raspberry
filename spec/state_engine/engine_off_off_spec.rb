...
describe StateEngine do

  context 'StateOffOff being the actual state' do

    init_state = StateOffOff.new
    engine = StateEngine.new(init_state)
    p engine
    ...
    describe 'changes to StateOffOn' do      
      ...
      it 'should notify> Cerradura Bloqueada,Puerta Violada' do
        off_on = StateOffOn.new
        response = engine.change_to(off_on)
        expected_response = 'Cerradura Bloqueada,Puerta Violada'
        expect(response.message).to eq expected_response
      end
    end
